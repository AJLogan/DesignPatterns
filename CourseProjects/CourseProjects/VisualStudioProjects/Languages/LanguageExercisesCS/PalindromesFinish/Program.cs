...
namespace PalindromesFinish {
    internal class Program {
        ...
        private static bool TestForPalindrome(string input) {
            string transformed = input.ToLower();
            int forward = 0;
            int backward = transformed.Length - 1;

            while (forward < backward) {
                while (!IsLetter(transformed[forward])) {
                    forward += 1;
                }
                while (!IsLetter(transformed[backward])) {
                    backward -= 1;
                }
                if (transformed[forward] != transformed[backward]) {
                    return false;
                }
                forward++;
                backward--;
